...
CACHE_FILE = os.path.join(CACHE_DIR, "s4_trades_cache.parquet")

def load_s4_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S4 trades from cache: {CACHE_FILE}")
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S4 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S4 trades: {e}")
    return df_out
