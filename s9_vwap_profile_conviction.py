...
"""
================================================================================
ENGINE 2: S15 - VWAP PROFILE CONVICTION (6R RUNNER GEOMETRY)
================================================================================
"""
...
logger = logging.getLogger("S15_VWAPProfile")
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s15")
...
def s15_signal_predicate(df):
    long_mask = (df['mc'] > 0.001) & (df['spot_cvd_delta'] > 0) & (df['p21'] > 0.001)
    short_mask = (df['mc'] < -0.001) & (df['spot_cvd_delta'] < 0) & (df['p21'] < -0.001)
    return long_mask, short_mask

CACHE_DIR = os.path.join(SCRIPT_DIR, "cache")
os.makedirs(CACHE_DIR, exist_ok=True)
CACHE_FILE = os.path.join(CACHE_DIR, "s15_trades_cache.parquet")

def load_s15_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S15 trades from cache: {CACHE_FILE}")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            mask_l, mask_s = s15_signal_predicate(df)
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S15 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S15 trades: {e}")
    return df_out
...
def run_s15_walkforward():
    ...
    df_all = load_s15_trades(feature_cols)
