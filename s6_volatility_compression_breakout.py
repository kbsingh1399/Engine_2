...
logger = logging.getLogger("S6_VolCompression")
...
def s6_signal_predicate(df):
    c = df['close']
    ma20 = c.rolling(20, min_periods=1).mean()
    std20 = c.rolling(20, min_periods=1).std().replace(0.0, 1e-8)
    upper_bb = ma20 + 2.0 * std20
    lower_bb = ma20 - 2.0 * std20
    bb_width = (upper_bb - lower_bb) / ma20
    bb_compression = bb_width < bb_width.rolling(96, min_periods=1).quantile(0.20)
    
    long_mask = bb_compression & (c > upper_bb.shift(1)) & (df['vol_ratio'] > 1.2)
    short_mask = bb_compression & (c < lower_bb.shift(1)) & (df['vol_ratio'] > 1.2)
    return long_mask, short_mask

CACHE_DIR = os.path.join(SCRIPT_DIR, "cache")
os.makedirs(CACHE_DIR, exist_ok=True)
CACHE_FILE = os.path.join(CACHE_DIR, "s6_trades_cache.parquet")

def load_s6_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S6 trades from cache: {CACHE_FILE}")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            ma20 = c.rolling(20, min_periods=1).mean()
            std20 = c.rolling(20, min_periods=1).std().replace(0.0, 1e-8)
            df['bb_width'] = ((ma20 + 2.0 * std20) - (ma20 - 2.0 * std20)) / ma20
            ...
            mask_l, mask_s = s6_signal_predicate(df)
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S6 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S6 trades: {e}")
    return df_out
...
if __name__ == "__main__":
    ...
    df = load_s6_trades(feature_cols)
    print(f"Loaded {len(df)} S6 trades.")
