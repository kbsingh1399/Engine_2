...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s5")
...
CACHE_FILE = os.path.join(CACHE_DIR, "s5_trades_cache.parquet")

def load_s5_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S5 trades from cache: {CACHE_FILE}")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            mask_l, mask_s = s5_signal_predicate(df)
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S5 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S5 trades: {e}")
    return df_out
