...
"""
================================================================================
ENGINE 2: S7 - DELTA CLIMAX MEAN REVERSION (CALIBRATED 1.4/32 PREDICATE)
================================================================================
"""
...
logger = logging.getLogger("S7_DeltaClimax")
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s7")
    ...
    for j in range(entry_idx + 1, max_idx):
        ...
        if direction == 1: # LONG
            ...
            if highs[j] > best_price:
                ...
                if gain >= 4.5 * stop_dist:
                    exit_price = entry_price + 4.5 * stop_dist
                    exit_offset = bars_held
                    break
                elif gain >= 3.0 * stop_dist:
                    new_stop = entry_price + 1.8 * stop_dist
                    if new_stop > cur_stop: cur_stop = new_stop
                elif gain >= 1.6 * stop_dist:
                    new_stop = entry_price + 0.35 * stop_dist
                    if new_stop > cur_stop: cur_stop = new_stop
        else: # SHORT
            if highs[j] >= cur_stop:
                exit_price = cur_stop * (1.0 + SLIPPAGE_PCT)
...
def s7_signal_predicate(df):
    long_mask = (df['zc4'] < -1.4) & (df['rsi'] < 32.0) & (df['vol_ratio'] > 1.2)
    short_mask = (df['zc4'] > 1.4) & (df['rsi'] > 68.0) & (df['vol_ratio'] > 1.2)
    return long_mask, short_mask

CACHE_DIR = os.path.join(SCRIPT_DIR, "cache")
os.makedirs(CACHE_DIR, exist_ok=True)
CACHE_FILE = os.path.join(CACHE_DIR, "s7_trades_cache.parquet")

def load_s7_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S7 trades from cache: {CACHE_FILE}")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            mask_l, mask_s = s7_signal_predicate(df)
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S7 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S7 trades: {e}")
    return df_out
...
if __name__ == "__main__":
    ...
    df = load_s7_trades(feature_cols)
    print(f"Loaded {len(df)} S7 trades.")
