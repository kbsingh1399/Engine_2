...
"""
================================================================================
ENGINE 2: S2 - CVD MOMENTUM STRATEGY (VERIFIED MOMENTUM GEOMETRY)
================================================================================
"""
...
logger = logging.getLogger("S2_CVDMom")
...
def s2_signal_predicate(df):
    zc4 = df.get('zc4', pd.Series(0.0, index=df.index)).fillna(0.0)
    zc20 = df.get('zc20', pd.Series(0.0, index=df.index)).fillna(0.0)
    vol = df.get('vol_ratio', pd.Series(1.0, index=df.index)).fillna(1.0)
    
    long_mom = (zc4 > 1.2) & (zc20 > 0.5) & (vol > 1.15)
    short_mom = (zc4 < -1.2) & (zc20 < -0.5) & (vol > 1.15)
    return long_mom, short_mom

CACHE_DIR = os.path.join(SCRIPT_DIR, "cache")
os.makedirs(CACHE_DIR, exist_ok=True)
CACHE_FILE = os.path.join(CACHE_DIR, "s2_trades_cache.parquet")

def load_s2_trades(feature_cols, force_recompute=False):
    if not force_recompute and os.path.exists(CACHE_FILE):
        try:
            logger.info(f"Loading S2 trades from cache: {CACHE_FILE}")
    ...
    try:
        ...
        logger.info(f"Cached {len(df_out)} S2 trades to {CACHE_FILE}")
    except Exception as e:
        logger.warning(f"Failed to cache S2 trades: {e}")
    return df_out
...
if __name__ == "__main__":
    ...
    df = load_s2_trades(feature_cols)
    print(f"Loaded {len(df)} S2 trades.")